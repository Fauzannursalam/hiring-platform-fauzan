	...
	return (
		<header className="bg-white border-b border-[#e0e0e0] px-6 py-4 flex justify-between items-center">
			...
			<div className="relative" ref={btnRef}>
				<button aria-label="Open user menu" onClick={() => setOpen((s) => !s)} className="w-10 h-10 rounded-full bg-gradient-to-br from-[#01959f] to-[#43936c] flex items-center justify-center text-white font-semibold cursor-pointer">
					{role?.[0]?.toUpperCase() ?? "A"}
				</button>

				{open && role && (
					<div className="absolute right-0 mt-2 w-40 bg-white border border-[#e6e6e6] rounded shadow-md z-50">
						<button onClick={handleLogout} className="w-full text-left px-3 py-2 hover:bg-[#f3f3f3]">
							Logout
						</button>
					</div>
				)}
